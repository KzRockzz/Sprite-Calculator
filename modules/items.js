...
// ---------- Keypad for dropdown long‑press (decimal capable) ----------
...
function showMultiplierPad(anchorBtn, baseValue, onApply){
  ...
  bd.addEventListener('click', close); [4]
...
function renderCards(mountEl, state, ui){
  ...
  filtered.forEach((it)=>{
    ...
    if(ui.openId===it.id){
      ...
      dd.style.cssText='margin-top:8px;border-radius:9px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);padding:8px'; [4]
      ...
      let lastMini = 0;
      const off = onMiniChange(v => { lastMini = v; if(mirror) mirror.innerHTML = formatMoney(v); }); [4]
      ...
      dd.addEventListener('click', (e)=>{
        ...
        if(close){
          off && off(); ui.openId=null; renderCards(mountEl,state,ui);
          return;
        }
      }); [4]
